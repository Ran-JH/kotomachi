...
function normalizeCard(raw: Record<string, unknown>): SessionSummaryApiCard {
  return {
    ...
    reviewWords: normalizeReviewWords(raw.reviewWords),
...
export async function POST(req: NextRequest) {
  try {
    ...
    return NextResponse.json({ card: normalizeCard(parseJsonObject(raw)) });
  } catch (error) {
