...
**Version:** 0.2 (partial implementation)  
**Status:** Partially implemented — welcome, starter prompts, home entry, life arc, and avatar are live; chat system prompt uses fallback; guided scenes not yet implemented  
...
| visible trait | 经常晚上出门，养着一只叫モク的猫头鹰 |
| owl name | モク |
| location | 夜路小巷（夜の路地 / Night Alley） |
| register | 柔らかい普通体 + 少し文学的 |
| tone | 安静、神秘、温柔、有一点奇怪，但不恐怖 |
| avatar asset | `/avatars/saku_avatar.webp` |
...
**Implementation note:** In the welcome API, Saku is described as having a small animal that collects shiny things — he "temporarily looks after it and cleans up after it" rather than directly calling it his pet. When pressed for a straight answer, he gives a normal explanation, then gently steers back to the current note, receipt, book, words, or memory.
...
During nighttime hours, a small entry card appears on the homepage hero/banner area:
...
**English:**
> Night rumor  
> Moku seems to be awake.  
> [Take a look →]

Clicking leads to `/chat/saku`.

### 2. Daytime Hidden Hotspot + Receipt Easter Egg

During daytime, no clickable entry is shown. However, there is a hidden hotspot on the homepage banner (around x: 1058, y: 362). Clicking this hotspot 3 times triggers a receipt easter egg:

- A receipt drops out showing "自販機レシート" with the line "「まだ言ってない。」"
- The receipt has a "拾起来 →" CTA that leads to `/chat/saku`
- URL params `?showReceipt=1` can force-show the receipt for testing
...
**Implementation note:** The entry logic is implemented in `components/home/rumor-entry.tsx` using `lib/rumor-gate.ts` for the time check (`isRumorHour`). URL params `?showRumor=1` and `?showHotspot=1` can force-show elements for testing.
...
| File | Changes |
|---|---|
| `lib/npc.ts` | saku added to NpcId union, NPC_NAMES, NPC_DISPLAY_NAMES, NPC_AVATARS, NPC_ARCS, HOME_CARD_LINES |
| `app/chat/[npcId]/page.tsx` | Routing works via /chat/saku |
| `app/api/welcome/route.ts` | Saku personality definition and welcome logic implemented |
| `components/home/scene-entry-section.tsx` | Saku info registered but does not appear in standard NPC lists |
| `components/home/rumor-entry.tsx` | Nighttime-only rumor entry card + daytime receipt easter egg |
| `lib/rumor-gate.ts` | isRumorHour(hour) utility for 20:00–05:00 time check |
| `lib/starter-prompts.ts` | Saku-specific starter prompts added |
| `public/avatars/` | saku_avatar.webp asset added |
| `lib/volcengine.ts` | Saku voice ID configured |
| `app/api/topic-ideas/route.ts` | Saku-specific topic ideas configured |
| `components/home/continue-section.tsx` | Saku appears in recent conversations after first visit |

### Not Yet Implemented

| File | Changes |
|---|---|
| `app/api/chat/route.ts` | Saku-specific system prompt branch — currently uses fallback prompt |
| `lib/conversation-scenes.ts` | Guided scenes not yet added |
| `lib/rumor-hints.ts` | Daytime ambient hint rotation — not yet implemented |

Implementation note: The welcome API has detailed Saku personality definition, including fallback welcome messages:
- 初回: "こんばんは。モクが、さっきから窓の外を見てる。たぶん、誰かが言い忘れた言葉を探してるんだと思う。"
- 再訪: "こんばんは。モクが今夜は静かなんです。何か、胸に残っていることがありますか。"
...
- [x] Add `saku` to `lib/npc.ts` types and maps
- [x] Create `lib/rumor-gate.ts` for time-based entry logic
- [x] Create `components/home/rumor-entry.tsx` for nighttime banner badge
- [x] Add `saku` to chat routing in `app/chat/[npcId]/page.tsx`
- [x] Add `saku` personality and welcome logic in `app/api/welcome/route.ts`
- [x] Add `saku` starter prompts in `lib/starter-prompts.ts`
- [x] Add `saku` avatar asset in `public/avatars/`
- [x] QA: Verify Saku does NOT appear in standard NPC sections
- [x] QA: Verify nighttime entry appears only between 20:00–05:00
- [x] QA: Verify `/chat/saku` is accessible via direct URL during daytime
...
- [ ] Create `lib/rumor-hints.ts` for daytime ambient hint rotation
