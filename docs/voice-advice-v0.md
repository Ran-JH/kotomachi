...
### Route A: True audio-aware model

将音频传给支持音频输入的模型或语音评估服务，返回听感建议。

优点：

- 真实分析音频；
- 和 Expression Hints 区分明显。
...
### Route B: Audio metadata + transcript assisted
...
- 不应声称做了精确发音诊断；
- 只能给粗粒度 speaking advice。

### Route C: Transcript-only fallback
...
v0 应优先探索 Route A 或 Route B。

Route C 可以作为安全兜底，但不能包装成真正的 audio-aware advice。
...
- 只在当前会话有 `userAudioBlob` 时显示
- 点击后调用 `/api/voice-advice`
- 显示 loading / error / result card
- 不保存，不收藏，不进 Review Card

### Pack 3: Audio-aware backend

- 接入真实 audio-aware model 或受控 audio metadata analysis
- 明确成本、延迟和失败态
- 保证不与 Expression Hints 混淆

### Pack 4: Polish after testing
...
- 使用哪一个 audio-aware backend？
- 是否需要音频时长 / 停顿 metadata？
- 是否允许 Route B 作为第一版？
- 如果 backend unavailable，UI 是否隐藏按钮还是显示 unavailable？
- 是否需要重新录一遍按钮？
- 是否要把 Voice Advice 放 drawer 还是 inline card？
- 是否未来接入 Review Card？
- 是否未来支持 activeScene？
- 成本是否可控？
- 移动端按钮是否拥挤？
