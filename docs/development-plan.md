...
- [x] expression hint quality upgrade — three-level semantics, shared notes, per-level notes, usage, structure note
- [x] expression hint saved item integration — saves level, usage, sharedRevisionNotes, revisionNotes, structureNote
- [x] expression hint learning labels fix — fixed undefined learningLabels and ???? placeholders
...
## 3. Expression Hint v0 — Implemented, in QA

Current status:

- Expression Hint now generates three Japanese alternatives for different relationship distances / tone levels.
- Product semantics are Casual / Neutral / Polite.
- Internally, legacy keys may still be `casual` / `business` / `formal`; `business` maps to Neutral and `formal` maps to Polite.
- The card separates:
  - "需要先调整的地方" (`sharedRevisionNotes`) for core fixes in the original expression;
  - "适合这样说" (`usage`) for when to use each level;
  - "这一档的表达差异" (`revisionNotes`) for level-specific register differences;
  - "表达结构" (`structureNote`) for reusable patterns, shown inside expanded explanation only.
- Saving an expression now preserves the selected level's learning explanation, including core fixes, level-specific notes, and reusable pattern when available.
- Provider failure / fallback handling has been hardened so failed generation should not be shown as a normal learning card.
- The previous `learningPoints` experiment has been removed.

QA stage:

- Continue manual testing on real voice-transcribed inputs.
- Collect bad cases where key fixes are missed or repeated.
- Avoid further large schema/UI changes unless repeated bad cases justify them.

Known deferred items:

- Some UI labels may still use historical wording such as "轻松闲聊 / 职场社交 / 正式交际"; product semantics are Casual / Neutral / Polite and can be copy-polished later.
- `structureNote` is not currently cached in Expression Hint cache; this can be added later if needed.

---

## 4. Next / 下一阶段

### 4.1 High-priority QA and polish
...
### 4.2 Product boundary protection
