...
> 文档用途：作为后续开发、Codex 协作、Vercel 上线、UI/UX 优化、产品迭代和面试复盘的统一参考。
...
**2026-05-27 状态更新：** 项目已经部署到 Vercel，当前阶段从本地 demo 推进为线上作品集 MVP。后续重点应从“继续增加功能”转为“稳定线上体验、记录真实使用问题、积累 prompt eval cases、整理面试叙事”。
...
> 稳定可部署、体验清楚、可长期迭代、可写进简历、可在面试中讲出完整产品与工程逻辑的状态。
...
Interview Story:
我发现语言学习 AI 容易把所有场景都变成教学，因此把社交回应和学习反馈拆成两个独立交互层。
```
...
后续简历中不要只写“使用 AI-assisted vibe coding 开发了网页”。更好的叙事是：

> 独立设计并实现一个 AI-native 日语口语互动产品，针对日语学习者“阅读能力强但口语输出焦虑”的问题，构建了低压力、渐进式、非纠错型的口语练习环境。项目采用 Next.js / TypeScript / Tailwind 实现街区地图、LINE 风格聊天、NPC 生活弧线、LocalStorage 记忆、多模型 fallback、TTS/STT 语音交互、划词查词与三档表达建议。通过将 NPC 社交聊天层与学习反馈层解耦，避免传统语言学习产品的考试化纠错体验，并通过 experience log 与 prompt eval 持续优化 AI 行为。
